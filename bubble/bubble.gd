...
func _collect() -> void:
	...
	tween.tween_property(self, "scale", 
		Vector2.ZERO, DURATION).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	tween.tween_property(self, "rotation", 
		7.0, DURATION).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	tween.tween_property(audio_stream_player, "volume_db", 
		-60, DURATION).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
